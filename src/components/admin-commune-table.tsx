...
type AdminCommuneItem = Commune & { documentCount: number };
type CommuneFilter = "all" | CommuneType;
type DataFilter = "all" | "has_docs" | "no_docs" | "missing_image" | "missing_keywords";
...
export function AdminCommuneTable({ communes }: { communes: AdminCommuneItem[] }) {
  ...
  const filteredCommunes = useMemo(() => {
    ...
    return communes.filter((commune) => {
      ...
      if (dataFilter === "has_docs" && !commune.documentCount) return false;
      if (dataFilter === "no_docs" && commune.documentCount) return false;
  ...
  return (
    <section className="mt-8 overflow-hidden rounded border border-ink/10 bg-white shadow-sm">
      <div className="border-b border-ink/8 bg-paper px-4 py-4">
        ...
        <p className="mt-3 text-sm text-ink/55">Đang hiển thị {filteredCommunes.length} / {communes.length} xã/phường</p>
      </div>
          ...
          <tbody className="divide-y divide-ink/8">
            {filteredCommunes.length ? (
              filteredCommunes.map((commune) => (
                <tr key={commune.id} className={cn(commune.documentCount ? "bg-palm/3" : "bg-white")}>
